...
class Prgramm
{
    static Container<IName>? containerArray = new Container<IName>();
    static ContainerLinkedList<IName>? containerList = new ContainerLinkedList<IName>();
    static ContainerType activeContainerType = ContainerType.None;
    
    static void Main()
    {
        var product = new Product("Gaming Mouse", 75.99m);

        // 2. RealEstate (inherits from Product)
        var realEstate = new RealEstate(
            "Downtown Office Space", // Name
            1250000m,                // Price
        ...
        // 3. RealEstateInvestment (inherits from Product)
        var realEstateInvestment = new RealEstateInvestment(
            "Beachfront Property Fund", // Name
            750000m,                    // Price (Purchase/Initial)
            "Sunrise Coast",            // Location
            810000m,                    // MarketValue
            "Vacation Rental"           // InvestmentType
        );

        // 4. Apartment (inherits from RealEstate)
        var apartment = new Apartment(
            "The Lofts Unit 3A",     // Name
            450000m,                // Price
            "456 Urban St",         // Location
            1100.5,                 // Size (double)
            "Condominium",          // Type
            3,                      // FloorNumber (int)
            550.75m                 // HOAFees (decimal)
        );

        // 5. House (inherits from RealEstate)
        var house = new House(
            "Oak Residence",          // Name
            620000m,                // Price
            "789 Suburban Ln",      // Location
            2400.0,                 // Size (double)
            "Single Family Home",   // Type
            1500.0,                 // GardenSize (double)
            true                    // Pool (bool) - Has a pool
        );

        // 6. Hotel (inherits from RealEstateInvestment)
        var hotel = new Hotel(
            "Grand Mountain Resort", // Name
            25000000m,               // Price (Purchase/Initial)
            "Alpine Valley",         // Location
            28500000m,               // MarketValue
            "Luxury Hospitality",    // InvestmentType
            300,                     // Rooms (int)
            5                        // StarRating (int)
        );

        // 7. LandPlot (inherits from RealEstateInvestment)
        var landPlot = new LandPlot(
            "Future Development Site", // Name
            300000m,                   // Price (Purchase/Initial)
            "Highway 5 Expansion Zone",// Location
            325000m,                   // MarketValue
            "Speculative",             // InvestmentType
            "Clay Loam",               // SoilType (string)
            false                      // InfrastructureAccess (bool) - No access yet
        );


        // --- Add instances to the containers ---

        // Add to ArrayList
        containerArray.Add(product);
        containerArray.Add(realEstate);
        containerArray.Add(realEstateInvestment);
        containerArray.Add(apartment);
        containerArray.Add(house);
        containerArray.Add(hotel);
        containerArray.Add(landPlot);

        // Add to List<object>
        containerList.AddLast(product);
        containerList.AddLast(realEstate);
        containerList.AddLast(realEstateInvestment);
        containerList.AddLast(apartment);
        containerList.AddLast(house);
        containerList.AddLast(hotel);
        containerList.AddLast(landPlot);

        Console.WriteLine("Array writed in " + ContainerSerializer.SerializeContainer(containerArray, "data"));
        Console.WriteLine("LinkedList writed in " + ContainerSerializer.SerializeContainer(containerList, "dataList"));

        Container<IName>? array = new Container<IName>();
        Console.WriteLine("Read data from 'data':");
        foreach(var item in ContainerSerializer.DeserializeContainer("data"))
        {
        ...
        Console.WriteLine("Read data from 'dataList':");
        ContainerLinkedList<IName> list = new ContainerLinkedList<IName>();
        foreach (var item in ContainerSerializer.DeserializeContainer("dataList"))
        {
            Console.WriteLine(item.ToString());
            list.AddLast(item);
        }
        ...
        Console.WriteLine("Read data from 'array':");
        ...
        Console.WriteLine("Read data from 'list':");
