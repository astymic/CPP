...
class Program
{
    ...
    // Comparison delegates for the new Sort 
    ...
    static void HandleGetElementByInsertionId()
    {
        ...
        if (int.TryParse(Console.ReadLine(), out int inputId) && inputId >= 1 && inputId <= maxId)
        {
            ...
            if (currentIndex == -1)
            {
                // This can happen if item was found by ID but its current position cannot be determined by reference
                // (e.g. if sorting changed order and insertion ID mapping is not perfectly in sync with current array order)
                Console.ForegroundColor = ConsoleColor.Yellow;
    ...
    static void HandleSortedPriceGenerator()
    {
        ...
        foreach (var item in sortedItems)
        {
            if (item != null)
            {
                // For generated sequences, the display ID is its order in the sequence.
                // Showing actual current index can be confusing as this is a *new* sequence.
    ...
    static void DemonstrateIndexersArray(Container<IName> container, Random random)
    {
        ...
        if (validDemoIdToSet != -1)
        {
            ...
            if (itemToModify != null)
            {
                ...
                try
                {
                    // Note: The indexer 'set' for Container<T> is tricky if it replaces the instance.
                    // The current Container<T>[id] setter attempts to modify the existing item IF FOUND,
                    // or throws if not found. It doesn't replace the item instance directly via the setter.
                    // This part demonstrates getting the item, then modifying its properties.
                    ...
                    // To demonstrate the 'set' indexer if it were to replace the item:
                    // var newItem = new Product(newName, itemToModify.Price); // Create a new item
                    // container[validDemoIdToSet] = newItem; // This would use the setter if it replaces
        ...
        if (validDemoIdListToSet != -1)
        {
            ...
            if (itemToModify != null)
            {
                ...
                try
                {
                    ...
                    // To demonstrate 'set' indexer for replacement:
                    // var newItem = new Product(newName, itemToModify.Price);
                    // container[validDemoIdListToSet] = newItem; // This uses the 'set' part of the indexer
    ...
    static int FindValidInsertionId(Container<IName> container)
    {
        ...
        // GetInsertionOrder returns all IDs ever assigned, even for removed items.
        // We need an ID that currently maps to an item.
        var currentInsertionIds = container.GetInsertionOrder().Take(container.GetCount()).ToList();
        if (currentInsertionIds.Any())
        {
            // Iterate backwards to prefer more recently added items for demo
    ...
    static int FindValidInsertionId(ContainerLinkedList<IName> container)
    {
        ...
        if (order.Any())
        {
            // Iterate backwards through the *current* insertion order list.
            // The list 'order' should only contain IDs of items currently in the list.
    ...
    static void ModifyProperty(object itemToModify, int itemInsertionId)
    {
        ...
        if (int.TryParse(Console.ReadLine(), out int propChoice) && propChoice >= 1 && propChoice <= properties.Count)
        {
            ...
            try
            {
                ...
                {
                    // If current index cannot be found, display with its (1-based) insertion ID as a fallback
    ...
    // Gets the 0-based insertion ID for a given item.
    // This is more complex due to sorting and removals.
    // The Container's GetInsertionOrder() returns a snapshot of IDs related to items
    // at the time of call or in their original add order, which might not align with current item order.
    private static int GetInsertionIdForItem(IName itemToFind)
    {
        if (itemToFind == null) return -1;

        // This method is tricky because the insertionOrder array in Container<T>
        // is reordered along with `items` during sorts.
        // For ContainerLinkedList, InsertionOrder list is also reordered.
        // So, we find the item's current index, then look up that index in the insertionOrder list/array.
    ...
    private static IName? GetItemByCurrentIndex(int index) // index is 0-based
    {
        ...
        if (activeContainerType == ContainerType.Array && containerArray != null)
        {
            if (index < containerArray.GetCount())
            {
                ...
                // Fallback if GetItems()[index] is how it's structured (less robust if sorts compact nulls)
                // return containerArray.GetItems()[index];
            }
        }
        else if (activeContainerType == ContainerType.LinkedList && containerList != null)
        {
    ...
    static void HandleDeserializeContainer()
    {
        ...
        try
        {
            // DeserializeContainer returns IEnumerable<IName?>, but it's actually the container instance.
            // This needs to be cast back to the specific container type.
            ...
            {
                // This case might occur if DeserializeContainer only returns a generic IEnumerable
                // and not the actual container object. If so, the deserializer needs adjustment
                // or this part needs to reconstruct the container.
                // For now, assume DeserializeContainer correctly returns the container instance.
    ...
    static House GenerateRandomHouse(Random random)
    {
        ...
        // Ensure GardenSize is not negative as per constructor validation.
    ...
    static LandPlot GenerateRandomLandPlot(Random random)
    {
        ...
        bool infra = random.Next(2) == 0; // 50/50 chance
        return new LandPlot(
            names[random.Next(names.Length)],
            Math.Max(0.01m, price),
            locations[random.Next(locations.Length)],
